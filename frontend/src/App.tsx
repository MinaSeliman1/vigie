...
import type { AuditEntryResponse, AuditPageResponse, AvailabilityResponse, CoverageResponse, CreateSiteInput, MembershipResponse, NotificationResponse, Role as ApiRole, SectorResponse, CertificationResponse, CertificationTypeResponse, SiteResponse, ShiftResponse, SwapRequestResponse, TeamAvailabilityResponse, UserSummary } from './api/types'
...
type Role = ApiRole
type View = 'calendar' | 'swaps' | 'certifications' | 'team' | 'sites' | 'coverage' | 'availability' | 'audit'
type AuthMode = 'login' | 'register' | 'accept-invitation' | 'password-reset-request' | 'password-reset-confirm'
...
type Shift = { id: string; assignmentId?: string; siteId?: string; startUtc?: string; endUtc?: string; dateKey?: string; day: string; date: number; start: string; end: string; site: string; siteKind: string; status: 'assigné' | 'disponible'; lifecycleStatus?: string; publicationStatus?: string; colleagues: string[]; requiredLifeguards: number; assignments?: Array<{ id: string; employeeId: string; employeeName: string }> }
...
type CertificationRow = { id: string; certificationTypeId?: string; employeeId?: string; initials: string; name: string; email: string; type: string; expiry: string; detail: string; warning: boolean }
type InviteDraft = { name: string; email: string; role: Role; siteId: string; sectorId: string }
type SiteDraft = Omit<CreateSiteInput, 'startMonth' | 'startDay' | 'endMonth' | 'endDay'> & { season: 'all-year' | 'seasonal'; startMonth: string; startDay: string; endMonth: string; endDay: string }
type SectorDraft = { name: string; code: string }
type AuditFilters = { q: string; action: string; entityType: string; from: string; to: string }
...
const organizationTypeOptions = [
  { value: 'municipal', label: 'Piscine municipale' },
  { value: 'private', label: 'Centre aquatique privé' },
  { value: 'school', label: 'Piscine scolaire' },
  { value: 'hotel', label: 'Hôtel ou résidence' },
  { value: 'other', label: 'Autre installation' },
]
const poolCountOptions = [
  { value: 'one', label: '1 piscine' },
  { value: 'two-to-five', label: '2 à 5 piscines' },
  { value: 'six-to-ten', label: '6 à 10 piscines' },
  { value: 'more-than-ten', label: 'Plus de 10 piscines' },
  { value: 'unknown', label: 'Je ne sais pas encore' },
]
const teamSizeOptions = [
  { value: 'one-to-ten', label: '1 à 10 personnes' },
  { value: 'eleven-to-thirty', label: '11 à 30 personnes' },
  { value: 'thirty-one-to-one-hundred', label: '31 à 100 personnes' },
  { value: 'more-than-one-hundred', label: 'Plus de 100 personnes' },
  { value: 'unknown', label: 'Je ne sais pas encore' },
]
const primaryGoalOptions = [
  { value: 'planning', label: 'Planifier les quarts' },
  { value: 'swaps', label: 'Gérer les remplacements' },
  { value: 'certifications', label: 'Suivre les certifications' },
  { value: 'all', label: 'Tout centraliser' },
]

function demoDate(offset: number) {
  const date = startOfWeekMonday(new Date())
  date.setDate(date.getDate() + offset)
  return { dateKey: localDateKey(date), day: frenchDays[date.getDay()], date: date.getDate() }
}

const initialShifts: Shift[] = [
  { id: 's1', ...demoDate(1), start: '09:00', end: '17:00', site: 'Piscine du Nord', siteKind: 'Intérieur', status: 'assigné', colleagues: ['NT', 'CG'], requiredLifeguards: 2 },
  { id: 's2', ...demoDate(2), start: '13:00', end: '21:00', site: 'Piscine du Nord', siteKind: 'Intérieur', status: 'disponible', colleagues: ['NT'], requiredLifeguards: 2 },
  { id: 's3', ...demoDate(4), start: '14:00', end: '22:00', site: 'Bassin du parc', siteKind: 'Extérieur', status: 'assigné', colleagues: ['SN', 'CG'], requiredLifeguards: 2 },
  { id: 's4', ...demoDate(5), start: '12:00', end: '20:00', site: 'Piscine du Nord', siteKind: 'Intérieur', status: 'disponible', colleagues: [], requiredLifeguards: 2 },
]
...
const auditActionLabels: Record<string, string> = {
  ...
  'certification.created': 'Certification ajoutée',
...
function formatCoverageDate(value: string) {
  return new Intl.DateTimeFormat('fr-CA', { weekday: 'long', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }).format(new Date(value))
}
...
function shiftDateLabel(shift: Pick<Shift, 'startUtc' | 'day' | 'date'>) {
  return shift.startUtc ? formatShiftDate(shift.startUtc) : `${shift.day} ${shift.date}`
}
...
function formatWeekEyebrow(start: Date) {
  const end = new Date(start)
  end.setDate(end.getDate() + 6)
  if (start.getMonth() === end.getMonth() && start.getFullYear() === end.getFullYear()) {
    const monthYear = new Intl.DateTimeFormat('fr-CA', { month: 'long', year: 'numeric' }).format(end)
    return `${start.getDate()} au ${end.getDate()} ${monthYear}`
  }
...
function readPasswordResetToken() {
  return typeof window === 'undefined' ? '' : new URLSearchParams(window.location.search).get('reset') ?? ''
}
...
function toUiShift(response: ShiftResponse, employeeId: string): Shift {
  ...
  return {
    ...
    publicationStatus: response.publicationStatus ?? 'Published',
...
function App() {
  ...
  const [authModal, setAuthModal] = useState<AuthMode | null>(() => readPasswordResetToken() ? 'password-reset-confirm' : readInvitationToken() ? 'accept-invitation' : null)
  const [invitationToken] = useState(readInvitationToken)
  const [passwordResetToken] = useState(readPasswordResetToken)
  ...
  const [authForm, setAuthForm] = useState({ organizationName: '', name: '', email: '', password: '', organizationType: '', poolCount: '', teamSize: '', primaryGoal: '' })
  ...
  const [membershipEditing, setMembershipEditing] = useState<MembershipResponse | null>(null)
  const [membershipSubmitting, setMembershipSubmitting] = useState(false)
  const [membershipError, setMembershipError] = useState('')
  const [membershipDraft, setMembershipDraft] = useState<{ role: Role; siteId: string; sectorId: string }>({ role: 'Lifeguard', siteId: '', sectorId: '' })
  ...
  const [certificationEditing, setCertificationEditing] = useState<CertificationResponse | null>(null)
  ...
  const [certificationTypes, setCertificationTypes] = useState<CertificationTypeResponse[]>([])
  const [certificationCreateOpen, setCertificationCreateOpen] = useState(false)
  const [certificationCreateDraft, setCertificationCreateDraft] = useState({ employeeId: '', certificationTypeId: '', expiresOn: '' })
  const [certificationCreateSubmitting, setCertificationCreateSubmitting] = useState(false)
  const [certificationCreateError, setCertificationCreateError] = useState('')
  ...
  const [teamAvailabilities, setTeamAvailabilities] = useState<TeamAvailabilityResponse[]>([])
  ...
  const [auditFilters, setAuditFilters] = useState<AuditFilters>({ q: '', action: '', entityType: '', from: '', to: '' })
  ...
  const [siteEditing, setSiteEditing] = useState<SiteResponse | null>(null)
  const [siteSubmitting, setSiteSubmitting] = useState(false)
  const [siteError, setSiteError] = useState('')
  const [siteDraft, setSiteDraft] = useState<SiteDraft>({ name: '', type: 'Indoor', timeZoneId: 'America/Toronto', season: 'all-year', startMonth: '6', startDay: '1', endMonth: '9', endDay: '1', address: '', neighborhood: '', isMunicipal: false, sectorId: '' })
  const [sectorModalOpen, setSectorModalOpen] = useState(false)
  const [sectorSubmitting, setSectorSubmitting] = useState(false)
  const [sectorError, setSectorError] = useState('')
  const [sectorDraft, setSectorDraft] = useState<SectorDraft>({ name: '', code: '' })
  ...
  const canManageOrganization = apiConfigured && currentUser.role === 'AquaticDirector'
  ...
  const teamMembers = employees
    ? isManagement && memberships ? employees.filter((employee) => memberships.some((membership) => membership.employeeId === employee.id && membership.isActive)) : employees
    : currentUser.isDemoAccount ? demoUsers.map(({ id, name, email, role }) => ({ id, name, email, role })) : []
  ...
  const weekDays = useMemo(() => Array.from({ length: 7 }, (_, index) => {
    ...
    return { date: localDateKey(date), day: frenchDays[date.getDay()], number: String(date.getDate()) }
  }), [calendarWeekStart])
  const calendarWeekLabel = useMemo(() => formatWeekRange(calendarWeekStart), [calendarWeekStart])
  const calendarFrom = calendarWeekStart.toISOString()
  ...
  useEffect(() => {
    ...
    async function syncApi() {
      try {
        ...
        const [apiShifts, apiSwaps, employees, apiCertifications, apiCertificationTypes, apiSites, apiAvailabilities, apiSectors, apiMembers, apiAudit, apiNotifications, apiCoverage, apiTeamAvailabilities] = await Promise.all([vigieApi.shifts(calendarFrom, calendarTo), vigieApi.swaps(), vigieApi.employees(), vigieApi.certifications(), vigieApi.certificationTypes(), vigieApi.sites(), vigieApi.availability(), vigieApi.sectors(), vigieApi.members(), isManagement ? vigieApi.auditSearch() : Promise.resolve({ items: [], total: 0, page: 1, pageSize: 20 } satisfies AuditPageResponse), vigieApi.notifications(), isManagement ? vigieApi.coverage(calendarFrom, calendarTo) : Promise.resolve([]), isManagement ? vigieApi.teamAvailability() : Promise.resolve([])])
        if (!isCurrent()) return
        ...
        setCertificationTypes(apiCertificationTypes)
        setSites(apiSites)
        setAvailabilities(apiAvailabilities)
        setTeamAvailabilities(apiTeamAvailabilities)
        setCoverage(apiCoverage)
        setAuditEntries(apiAudit.items)
        setAuditTotal(apiAudit.total)
        setAuditPage(apiAudit.page)
    ...
    void syncApi()
    return () => { active = false }
  }, [authBootstrapped, calendarFrom, calendarTo, currentUserEmail, currentUserId, currentUserIsDemo, currentUserName, currentUserOrganizationId, currentUserRole, currentUser.siteId, currentUser.sectorId, isManagement])
  ...
  function resetCalendarWeek() { setCalendarWeekStart(startOfWeekMonday(new Date())) }
  ...
  function openCertificationEditor(certification: CertificationResponse) {
    const canEdit = certification.employeeId === currentUser.apiId || isManagement
    if (!apiConfigured || certifications === null || !canEdit) {
      flash('La mise à jour des certifications est disponible pour un compte connecté.')
      return
    }
    setCertificationEditing(certification)
    setCertificationDraft(certification.expiresOn)
  ...
  async function submitCertification(event: FormEvent<HTMLFormElement>) {
    ...
    if (!certificationEditing || certificationSubmitting) return
    if (!certificationDraft) { setCertificationError('Choisissez une date d’expiration.'); return }
    setCertificationSubmitting(true)
    setCertificationError('')
    try {
      const updated = await vigieApi.saveCertification(certificationEditing.employeeId, certificationEditing.certificationTypeId, certificationDraft)
      setCertifications((items) => items?.map((item) => item.id === updated.id ? updated : item) ?? [updated])
      setCertificationEditing(null)
      flash(`Certification de ${updated.employeeName} mise à jour`)
    } catch (error) {
      setCertificationError(error instanceof Error ? error.message : 'La certification ne peut pas être mise à jour.')
    } finally {
      setCertificationSubmitting(false)
    }
  }
  function openCreateCertification() {
    if (!isManagement || !apiConfigured) {
      flash('L’ajout d’une certification est réservé aux responsables connectés.')
      return
    }
    const employee = teamMembers.find((member) => member.role === 'Lifeguard')
    const certificationType = certificationTypes[0]
    if (!employee || !certificationType) {
      flash('Ajoutez d’abord un sauveteur et un type de certification.')
      return
    }
    const date = new Date()
    date.setFullYear(date.getFullYear() + 1)
    setCertificationCreateDraft({ employeeId: employee.id, certificationTypeId: certificationType.id, expiresOn: date.toISOString().slice(0, 10) })
    setCertificationCreateError('')
    setCertificationCreateOpen(true)
  }
  function closeCreateCertification() {
    if (certificationCreateSubmitting) return
    setCertificationCreateOpen(false)
    setCertificationCreateError('')
  }
  async function submitCreateCertification(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (certificationCreateSubmitting) return
    const { employeeId, certificationTypeId, expiresOn } = certificationCreateDraft
    if (!employeeId || !certificationTypeId || !expiresOn) {
      setCertificationCreateError('Sélectionnez un membre, une certification et une date.')
      return
    }
    setCertificationCreateSubmitting(true)
    setCertificationCreateError('')
    try {
      const saved = await vigieApi.saveCertification(employeeId, certificationTypeId, expiresOn)
      setCertifications((items) => items?.some((item) => item.id === saved.id) ? items.map((item) => item.id === saved.id ? saved : item) : [...(items ?? []), saved])
      setCertificationCreateOpen(false)
      flash(`Certification de ${saved.employeeName} enregistrée`)
    } catch (error) {
      setCertificationCreateError(error instanceof Error ? error.message : 'La certification ne peut pas être enregistrée.')
    } finally {
      setCertificationCreateSubmitting(false)
    }
  }
  ...
  async function searchAudit(nextPage = 1, filters: AuditFilters = auditFilters) {
    if (!apiConfigured || auditLoading) return
    setAuditLoading(true)
    try {
      const result = await vigieApi.auditSearch({ ...filters, page: nextPage, pageSize: 20 })
      setAuditEntries(result.items)
      setAuditTotal(result.total)
      setAuditPage(result.page)
    } catch (error) {
      flash(error instanceof Error ? error.message : 'La recherche ne peut pas être effectuée.')
    } finally {
      setAuditLoading(false)
    }
  }
  function selectUser(id: string) { const user = demoUsers.find((candidate) => candidate.id === id); if (user) { syncGeneration.current += 1; localStorage.removeItem('vigie.token'); if (apiConfigured) setApiState('loading'); setSites([]); setSectors([]); setEmployees(null); setMemberships(null); setCertifications(null); setAvailabilities(null); setTeamAvailabilities([]); setCoverage([]); setAuditEntries([]); setAuditTotal(0); setAuditPage(1); setNotifications(null); setNotificationOpen(false); setCurrentUser(user); setView('calendar'); flash(`Profil de démonstration : ${user.name}`) } }
  function openAuth(mode: AuthMode) { setAuthError(''); setAuthForm({ organizationName: '', name: '', email: '', password: '', organizationType: '', poolCount: '', teamSize: '', primaryGoal: '' }); setAuthModal(mode) }
  ...
  async function submitAuth(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (mode === 'password-reset-request') {
        const response = await vigieApi.requestPasswordReset(authForm.email)
        setAuthModal(null)
        flash(response.message)
        return
      }
      if (mode === 'password-reset-confirm') {
        const response = await vigieApi.confirmPasswordReset(passwordResetToken, authForm.password)
        window.history.replaceState({}, document.title, window.location.pathname)
        setAuthForm({ organizationName: '', name: '', email: '', password: '', organizationType: '', poolCount: '', teamSize: '', primaryGoal: '' })
        setAuthModal('login')
        flash(response.message)
        return
      }
      const result = mode === 'register'
        ? (await vigieApi.register(authForm.organizationName, authForm.name, authForm.email, authForm.password, { organizationType: authForm.organizationType, poolCount: authForm.poolCount, teamSize: authForm.teamSize, primaryGoal: authForm.primaryGoal })).login
  ...
  async function exportAccountData() {
    ...
    try {
      const blob = await vigieApi.exportAccount()
      const url = URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = url
      link.download = `vigie-donnees-${new Date().toISOString().slice(0, 10)}.json`
      link.click()
      URL.revokeObjectURL(url)
      flash('Vos données ont été téléchargées.')
    } catch (error) {
      flash(error instanceof Error ? error.message : 'L’export de vos données a échoué.')
    } finally {
  ...
  async function deleteAccount() {
    ...
    if (!passwordForm.current) { setPasswordError('Saisissez votre mot de passe actuel avant de supprimer le compte.'); return }
    const confirmation = window.prompt('Cette action est irréversible. Tapez SUPPRIMER pour confirmer.')
    if (confirmation !== 'SUPPRIMER') return
    setAccountDeleting(true); setPasswordError('')
    try {
      await vigieApi.deleteAccount(passwordForm.current, confirmation)
      localStorage.removeItem('vigie.token')
      setCurrentUser(demoUsers[0]); setPasswordModalOpen(false); setAuthBootstrapped(true); setApiState(apiConfigured ? 'loading' : 'inactive')
      flash('Votre compte a été supprimé.')
    } catch (error) { setPasswordError(error instanceof Error ? error.message : 'Le compte ne peut pas être supprimé.') }
  ...
  function openCreateSite() {
    if (!canManageOrganization) { flash('La gestion des piscines est réservée à la régie aquatique.'); return }
    setSiteError('')
    setSiteEditing(null)
    setSiteDraft({ name: '', type: 'Indoor', timeZoneId: 'America/Toronto', season: 'all-year', startMonth: '6', startDay: '1', endMonth: '9', endDay: '1', address: '', neighborhood: '', isMunicipal: false, sectorId: '' })
    setSiteModalOpen(true)
  }
  function openEditSite(site: SiteResponse) {
    if (!canManageOrganization) { flash('La gestion des piscines est réservée à la régie aquatique.'); return }
    const allYear = site.openingSeason.startMonth === 1 && site.openingSeason.startDay === 1 && site.openingSeason.endMonth === 12 && site.openingSeason.endDay === 31
    setSiteError('')
    setSiteEditing(site)
    setSiteDraft({ name: site.name, type: site.type === 'Outdoor' ? 'Outdoor' : 'Indoor', timeZoneId: site.timeZoneId, season: allYear ? 'all-year' : 'seasonal', startMonth: String(site.openingSeason.startMonth), startDay: String(site.openingSeason.startDay), endMonth: String(site.openingSeason.endMonth), endDay: String(site.openingSeason.endDay), address: site.address ?? '', neighborhood: site.neighborhood ?? '', isMunicipal: site.isMunicipal ?? false, sectorId: site.sectorId ?? '' })
    setSiteModalOpen(true)
  }
  ...
  async function submitSite(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (!canManageOrganization || siteSubmitting) return
    setSiteSubmitting(true)
    setSiteError('')
    try {
      const name = siteDraft.name.trim()
      if (name.length < 2) throw new Error('Saisissez le nom de la piscine.')
      if (!siteDraft.timeZoneId) throw new Error('Choisissez le fuseau horaire de la piscine.')
      const season = siteDraft.season === 'all-year'
        ? { startMonth: 1, startDay: 1, endMonth: 12, endDay: 31 }
        : { startMonth: Number(siteDraft.startMonth), startDay: Number(siteDraft.startDay), endMonth: Number(siteDraft.endMonth), endDay: Number(siteDraft.endDay) }
      for (const [label, month, day] of [['début', season.startMonth, season.startDay], ['fin', season.endMonth, season.endDay]] as const) {
        if (!Number.isInteger(month) || !Number.isInteger(day) || month < 1 || month > 12 || day < 1 || day > new Date(2024, month, 0).getDate()) {
          throw new Error(`La date de ${label} de la saison est invalide.`)
        }
      }
      const input = { name, type: siteDraft.type, timeZoneId: siteDraft.timeZoneId, ...season, address: siteDraft.address?.trim() ?? '', neighborhood: siteDraft.neighborhood?.trim() ?? '', isMunicipal: siteDraft.isMunicipal, sectorId: siteDraft.sectorId || null }
      const saved = siteEditing ? await vigieApi.updateSite(siteEditing.id, input) : await vigieApi.createSite(input)
      setSites((items) => (siteEditing ? items.map((item) => item.id === saved.id ? saved : item) : [...items, saved]).sort((left, right) => left.name.localeCompare(right.name, 'fr')))
      setSiteModalOpen(false)
      setSiteEditing(null)
      flash(siteEditing ? `${saved.name} a été mise à jour.` : `${saved.name} a été ajoutée à votre catalogue.`)
    } catch (error) {
      setSiteError(error instanceof Error ? error.message : 'La piscine ne peut pas être ajoutée.')
    } finally {
      setSiteSubmitting(false)
    }
  }
  function openCreateSector() {
    if (!canManageOrganization) { flash('La gestion des secteurs est réservée à la régie aquatique.'); return }
    setSectorError('')
    setSectorDraft({ name: '', code: '' })
    setSectorModalOpen(true)
  }
  function closeCreateSector() {
    if (sectorSubmitting) return
    setSectorModalOpen(false)
    setSectorError('')
  }
  async function submitSector(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (!canManageOrganization || sectorSubmitting) return
    setSectorSubmitting(true)
    setSectorError('')
    try {
      const name = sectorDraft.name.trim()
      const code = sectorDraft.code.trim().toUpperCase()
      if (name.length < 2) throw new Error('Saisissez le nom du secteur.')
      if (!/^[A-Z0-9][A-Z0-9_-]{1,11}$/.test(code)) throw new Error('Le code doit contenir 2 à 12 caractères (lettres, chiffres, tiret ou souligné).')
      const created = await vigieApi.createSector({ name, code })
      setSectors((items) => [...items, created].sort((left, right) => left.name.localeCompare(right.name, 'fr')))
      setSectorModalOpen(false)
      flash(`${created.name} a été ajouté à votre organisation.`)
    } catch (error) {
      setSectorError(error instanceof Error ? error.message : 'Le secteur ne peut pas être ajouté.')
    } finally {
      setSectorSubmitting(false)
    }
  }
  function openMembershipEditor(membership: MembershipResponse) {
    setMembershipEditing(membership)
    setMembershipError('')
    setMembershipDraft({ role: membership.role, siteId: membership.siteId ?? '', sectorId: membership.sectorId ?? '' })
  }
  ...
  async function submitMembership(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (!membershipEditing || membershipSubmitting) return
    setMembershipSubmitting(true)
    setMembershipError('')
    try {
      const needsSite = membershipDraft.role === 'Lifeguard' || membershipDraft.role === 'PoolChief'
      const needsSector = membershipDraft.role === 'SectorManager'
      if (needsSite && !membershipDraft.siteId) throw new Error('Choisissez la piscine de rattachement.')
      if (needsSector && !membershipDraft.sectorId) throw new Error('Choisissez le secteur de rattachement.')
      const updated = await vigieApi.updateMembership(membershipEditing.id, {
        role: membershipDraft.role,
        siteId: needsSite ? membershipDraft.siteId : null,
        sectorId: needsSector ? membershipDraft.sectorId : null,
        isActive: true,
        expectedVersion: membershipEditing.version,
      })
      setMemberships((current) => current?.map((membership) => membership.id === updated.id ? updated : membership) ?? [updated])
      setMembershipEditing(null)
      flash(`Rattachement de ${updated.employeeName} mis à jour`)
    } catch (error) { setMembershipError(error instanceof Error ? error.message : 'Le rattachement ne peut pas être mis à jour.') }
    finally { setMembershipSubmitting(false) }
  }
  async function deactivateMembership(membership: MembershipResponse) {
    if (membership.employeeId === currentUser.apiId || membershipSubmitting) return
    if (!window.confirm(`Désactiver l’accès de ${membership.employeeName} à Vigie ?`)) return
    setMembershipSubmitting(true)
    try {
      await vigieApi.deactivateMembership(membership.id)
      setMemberships((current) => current?.map((item) => item.id === membership.id ? { ...item, isActive: false } : item) ?? current)
      flash(`Accès de ${membership.employeeName} désactivé`)
    } catch (error) { flash(error instanceof Error ? error.message : 'L’accès ne peut pas être désactivé.') }
    finally { setMembershipSubmitting(false) }
  }
  ...
  async function publishSelectedShift() {
    if (!selectedShift || shiftActionSubmitting || selectedShift.publicationStatus === 'Published' || !apiConfigured || apiState !== 'ready') return
    setShiftActionSubmitting(true)
    try {
      const published = await vigieApi.publishShift(selectedShift.id)
      const next = toUiShift(published, currentUser.apiId ?? currentUser.id)
      setShifts((items) => items.map((item) => item.id === next.id ? next : item))
      setSelectedShift(next)
      flash('Quart publié : il est maintenant visible par les sauveteurs')
    } catch (error) {
      flash(error instanceof Error ? error.message : 'Le quart ne peut pas être publié.')
    } finally {
      setShiftActionSubmitting(false)
    }
  }
  ...
  async function submitCreateShift(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setCreateShiftOpen(false)
      flash(apiConfigured && apiState === 'ready' ? 'Brouillon créé. Publiez-le quand il est prêt.' : 'Quart créé et ajouté au calendrier')
    } catch (error) {
  ...
  async function createSwap(receiver: string) {
    ...
    setSwaps((current) => [...current, { id: `swap-${Date.now()}`, shiftId: selectedShift.id, shiftLabel: `${shiftDateLabel(selectedShift)} · ${selectedShift.start}–${selectedShift.end}`, requester: currentUser.name, receiver, status: 'En attente' }])
    setSwapModalOpen(false); setSelectedShift(null); flash('Demande envoyée au coordonnateur')
  }
  ...
  const certificationRows: CertificationRow[] = certifications === null
    ...
    : certifications.map((certification) => {
      ...
      return {
        id: certification.id, certificationTypeId: certification.certificationTypeId, employeeId: certification.employeeId, initials: user?.initials ?? initials(certification.employeeName), name: certification.employeeName,
  ...
  const uncoveredShifts = coverage.filter((shift) => shift.publicationStatus === 'Published' && !shift.isCovered)
  const draftShifts = coverage.filter((shift) => shift.publicationStatus === 'Draft')
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileNavOpen ? 'is-open' : ''}`}>
      ...
      <nav aria-label="Navigation principale">
        ...
        {isManagement && <button className={view === 'coverage' ? 'nav-item active' : 'nav-item'} onClick={() => { setView('coverage'); setMobileNavOpen(false) }}><Icon name="users" /><span>Couverture</span>{uncoveredShifts.length > 0 && <b className="nav-count">{uncoveredShifts.length}</b>}</button>}
    ...
    <main className="main-content">
      ...
      <div className="content-wrap">
        <div className="page-heading"><div><p className="eyebrow">SEMAINE DU {formatWeekEyebrow(calendarWeekStart).toUpperCase()}</p><h1>{view === 'calendar' ? (isManagement ? 'Vue équipe' : `Bonjour, ${currentUser.name.split(' ')[0]}`) : view === 'swaps' ? 'Demandes d’échange' : view === 'certifications' ? 'Certifications' : view === 'team' ? 'Équipe' : view === 'sites' ? 'Piscines municipales' : view === 'coverage' ? 'Couverture des quarts' : view === 'audit' ? 'Historique' : 'Disponibilités'}</h1><p className="page-subtitle">{view === 'calendar' ? (isManagement ? 'Gardez une vue claire sur les quarts et les remplacements.' : 'Voici vos quarts et les disponibilités de votre équipe.') : view === 'swaps' ? 'Chaque remplacement reste sous contrôle jusqu’à votre approbation.' : view === 'certifications' ? 'La bonne certification, au bon moment, pour chaque quart.' : view === 'team' ? 'Les personnes autorisées et leur état de préparation pour les quarts.' : view === 'sites' ? 'Les installations, secteurs et saisons d’ouverture de votre organisation.' : view === 'coverage' ? 'Identifiez les besoins à combler avant la prise de service.' : view === 'audit' ? 'Les opérations importantes de votre organisation, avec leur acteur et leur horodatage.' : 'Indiquez les jours où vous pouvez prendre un quart.'}</p></div>{view === 'calendar' && isManagement && <button className="primary-button" onClick={openCreateShift}><Icon name="plus" />Créer un quart</button>}{view === 'team' && isManagement && <button className="primary-button" onClick={openInvite}><Icon name="plus" />Inviter un membre</button>}{view === 'sites' && canManageOrganization && <button className="primary-button" onClick={openCreateSite}><Icon name="plus" />Ajouter une piscine</button>}</div>
        {view === 'calendar' && <><section className="alert-banner"><div className="alert-icon"><Icon name="shield" /></div><div><strong>Certification à surveiller</strong><span>La certification Premiers soins de {currentUser.id === 'sofia' ? 'Sofia Nguyen' : 'votre dossier'} expire dans {currentUser.id === 'sofia' ? 20 : 75} jours.</span></div><button onClick={() => setView('certifications')}>Voir les détails <Icon name="arrow" /></button></section><div className="metric-row"><div className="metric"><span className="metric-label">QUARTS CETTE SEMAINE</span><strong>{assigned.length}</strong><span className="metric-meta positive">Votre planning publié</span></div><div className="metric"><span className="metric-label">HEURES PLANIFIÉES</span><strong>{assigned.reduce((total, shift) => { if (shift.startUtc && shift.endUtc) return total + (new Date(shift.endUtc).getTime() - new Date(shift.startUtc).getTime()) / 3600000; const [startHour, startMinute] = shift.start.split(':').map(Number); const [endHour, endMinute] = shift.end.split(':').map(Number); return total + ((endHour * 60 + endMinute) - (startHour * 60 + startMinute)) / 60 }, 0).toLocaleString('fr-CA', { maximumFractionDigits: 1 })}<span className="metric-unit">h</span></strong><span className="metric-meta">Quota hebdomadaire de votre profil</span></div><div className="metric"><span className="metric-label">ÉCHANGES EN ATTENTE</span><strong>{pendingSwaps.length}</strong><span className="metric-meta">Dernière mise à jour il y a 8 min</span></div></div><section className="calendar-section"><div className="section-header"><div><h2>{calendarWeekStart.getTime() === startOfWeekMonday(new Date()).getTime() ? 'Cette semaine' : 'Planning de la semaine'}</h2><p>Du {calendarWeekLabel}</p></div><div className="week-controls"><button aria-label="Semaine précédente" onClick={() => moveCalendarWeek(-1)}>←</button><button className="today-button" onClick={resetCalendarWeek}>Aujourd’hui</button><button aria-label="Semaine suivante" onClick={() => moveCalendarWeek(1)}>→</button></div></div><div className="calendar-grid">{shifts.length === 0 && <div className="calendar-empty-state" role="status"><strong>Aucun quart publié cette semaine</strong><span>{isManagement ? 'Créez un quart ou consultez une autre semaine pour poursuivre la planification.' : 'Les prochains quarts apparaîtront ici dès leur publication.'}</span><button type="button" onClick={() => moveCalendarWeek(1)}>Voir la semaine suivante <Icon name="arrow" /></button></div>}<div className="time-column"><span /><span>08:00</span><span>12:00</span><span>16:00</span><span>20:00</span></div>{weekDays.map((day) => <div className={`day-column ${day.date === localDateKey(new Date()) ? 'today' : ''}`} key={day.date}><div className="day-header"><span>{day.day}</span><strong>{day.number}</strong></div><div className="day-body">{shifts.filter((shift) => shift.dateKey === day.date || !shift.dateKey && shift.day === day.day).map((shift) => <button className={`shift-block ${shift.status}`} key={shift.id} onClick={() => setSelectedShift(shift)}><span className="shift-time">{shift.start} – {shift.end}</span><strong>{shift.site}</strong><small>{shift.siteKind}</small><span className="shift-people">{shift.colleagues.map((colleague) => <i key={colleague}>{colleague}</i>)}</span></button>)}</div></div>)}</div></section><div className="lower-grid"><section className="list-section"><div className="section-header compact"><div><h2>Échanges en attente</h2><p>Les demandes à traiter</p></div><button className="text-button" onClick={() => setView('swaps')}>Tout voir <Icon name="arrow" /></button></div>{pendingSwaps.length === 0 ? <div className="empty-state">Aucune demande en attente.</div> : pendingSwaps.map((swap) => <div className="swap-row" key={swap.id}><div className="mini-avatar">{swap.requester.split(' ').map((part) => part[0]).join('')}</div><div className="swap-copy"><strong>{swap.requester} <span>→</span> {swap.receiver}</strong><span>{swap.shiftLabel}</span></div><span className="pending-label">En attente</span></div>)}</section><section className="list-section"><div className="section-header compact"><div><h2>À venir</h2><p>Prochains quarts assignés</p></div><div>{shifts.filter((shift) => shift.status === 'assigné').length === 0 && <span className="empty-inline">Aucun quart assigné à venir.</span>}</div></div>{shifts.filter((shift) => shift.status === 'assigné').slice(0, 2).map((shift) => <div className="upcoming-row" key={shift.id}><div className="date-block"><strong>{shift.date}</strong><span>{shift.day}</span></div><div><strong>{shift.site}</strong><span>{shift.start} – {shift.end}</span></div><span className="assigned-mark"><Icon name="check" /></span></div>)}</section></div></>}
        ...
        {view === 'certifications' && <section className="full-section certification-page">
          <div className="section-header compact"><div><h2>Registre des certifications</h2><p>Vérifiez les échéances avant chaque assignation.</p></div>{isManagement && apiConfigured && <button className="primary-button" type="button" onClick={openCreateCertification}><Icon name="plus" />Ajouter une certification</button>}</div>
          ...
          <div className="cert-list">{certificationRows.map((certification) => <div className="cert-row" key={certification.id}><span className="mini-avatar">{certification.initials}</span><div className="cert-person"><strong>{certification.name}</strong><span>{certification.email}</span></div><div className="cert-name"><strong>{certification.type}</strong><span>Certification requise</span></div><div className={`cert-expiry ${certification.warning ? 'warning' : ''}`}><strong>{certification.expiry}</strong><span>{certification.detail}</span></div><span className={`cert-status ${certification.warning ? 'warning' : 'valid'}`}>{certification.warning ? 'À surveiller' : 'À jour'}</span>{apiConfigured && certification.employeeId && certification.certificationTypeId && (isManagement || certification.employeeId === currentUser.apiId) && <button className="detail-button" type="button" onClick={() => { const source = certifications?.find((item) => item.id === certification.id); if (source) openCertificationEditor(source) }}>Modifier</button>}</div>)}</div>
        </section>}
        {view === 'team' && <section className="full-section team-page">
          <div className="team-summary"><div><span className="metric-label">MEMBRES ACTIFS</span><strong>{teamMembers.length}</strong><p>Profils chargés depuis l’équipe Vigie.</p></div><div><span className="metric-label">RESPONSABLES</span><strong>{teamMembers.filter((member) => (memberships?.find((membership) => membership.employeeId === member.id && membership.isActive)?.role ?? member.role) !== 'Lifeguard').length}</strong><p>Accès à la planification selon leur périmètre.</p></div><div><span className="metric-label">SECTEURS ACTIFS</span><strong>{sectors.filter((sector) => sector.isActive).length || 2}</strong><p>Rattachements opérationnels de la régie.</p></div></div>
          <div className="sector-strip"><div><strong>Périmètres de la régie</strong><span>Chaque piscine peut être découpée en secteurs opérationnels.</span>{canManageOrganization && <button className="inline-action" type="button" onClick={openCreateSector}><Icon name="plus" />Ajouter un secteur</button>}</div><div className="sector-list">{(sectors.length > 0 ? sectors : [{ id: 'demo-nord', name: 'Secteur Nord', code: 'NORD', isActive: true } as SectorResponse]).map((sector) => <span className="sector-chip" key={sector.id}><i />{sector.name}<small>{sector.code}</small></span>)}</div></div>
          <div className="team-list"><div className="team-list-head"><span>MEMBRE</span><span>RÔLE</span><span>PÉRIMÈTRE</span><span>CONTACT</span><span>ACTIONS</span></div>{teamMembers.map((member) => { const memberCertifications = certificationRows.filter((certification) => certification.name === member.name); const hasWarning = memberCertifications.some((certification) => certification.warning); const certificationClass = memberCertifications.length === 0 || hasWarning ? 'warning' : 'valid'; const membership = memberships?.find((item) => item.employeeId === member.id && item.isActive); const memberRole = membership?.role ?? member.role; return <div className="team-row" key={member.id}><div className="person-cell"><span className="mini-avatar">{initials(member.name)}</span><div><strong>{member.name}</strong><small>{member.email}</small></div></div><span className="role-pill">{roleLabel(memberRole)}</span><span className={`cert-status ${certificationClass}`}>{membership ? roleScopeLabel(memberRole, membership.siteName, membership.sectorName) : memberCertifications.length === 0 ? 'À vérifier' : hasWarning ? 'À surveiller' : 'À jour'}</span><a className="team-email" href={`mailto:${member.email}`}>{member.email}</a>{isManagement && membership && member.id !== currentUser.apiId ? <div className="team-actions"><button className="detail-button" type="button" onClick={() => openMembershipEditor(membership)}>Modifier</button><button className="danger-button" type="button" onClick={() => { void deactivateMembership(membership) }}>Désactiver</button></div> : <div className="team-actions-placeholder" />}</div> })}</div>
        </section>}
        {view === 'sites' && <section className="full-section sites-page"><div className="sites-overview"><div><span className="metric-label">INSTALLATIONS VISIBLES</span><strong>{availableSites.length}</strong><p>Selon le périmètre de {roleLabel(currentUser.role).toLowerCase()}.</p></div><div><span className="metric-label">INTÉRIEURES</span><strong>{availableSites.filter((site) => site.type === 'Indoor').length}</strong><p>Ouvertes toute l’année selon le calendrier du site.</p></div><div><span className="metric-label">EXTÉRIEURES</span><strong>{availableSites.filter((site) => site.type === 'Outdoor').length}</strong><p>Saison estivale, ouverture contrôlée par la régie.</p></div></div><div className="sites-toolbar"><label className="site-search"><span>Rechercher une piscine</span><input aria-label="Rechercher une piscine" value={siteQuery} onChange={(event) => setSiteQuery(event.target.value)} placeholder="Nom, adresse ou quartier" /></label><label className="site-type-filter"><span>Type</span><select aria-label="Filtrer les piscines par type" value={siteTypeFilter} onChange={(event) => setSiteTypeFilter(event.target.value as SiteTypeFilter)}><option value="all">Tous les types</option><option value="Indoor">Intérieures</option><option value="Outdoor">Extérieures</option></select></label><span className="site-result-count">{filteredSites.length} résultat{filteredSites.length === 1 ? '' : 's'}</span></div><div className="sites-grid">{filteredSites.length === 0 ? <div className="empty-state">Aucune piscine ne correspond à vos filtres.</div> : filteredSites.map((site) => <article className="site-card" key={site.id}><div className={`site-icon ${site.type === 'Outdoor' ? 'outdoor' : ''}`}><Icon name="shield" /></div><div className="site-card-copy"><div className="site-card-title"><h2>{site.name}</h2><span>{site.type === 'Outdoor' ? 'Extérieure' : 'Intérieure'}</span></div><p>{site.address || 'Adresse à compléter'}{site.neighborhood ? ` · ${site.neighborhood}` : ''}</p><small>{site.type === 'Outdoor' ? 'Saison estivale' : 'Ouverte toute l’année'} · {site.isMunicipal ? 'Catalogue municipal Laval' : 'Site de démonstration'} · {site.sectorName ?? 'Secteur à assigner'}</small>{canManageOrganization && <button className="inline-action site-edit-action" type="button" onClick={() => openEditSite(site)}>Modifier la piscine <Icon name="arrow" /></button>}</div></article>)}</div></section>}
        {view === 'coverage' && <section className="full-section coverage-page"><div className="coverage-summary"><div><span className="metric-label">QUARTS SUR LA PÉRIODE</span><strong>{coverage.length}</strong><p>Les quarts actifs visibles dans votre périmètre.</p></div><div><span className="metric-label">COUVERTURE À RISQUE</span><strong className={uncoveredShifts.length > 0 ? 'coverage-number warning' : 'coverage-number'}>{uncoveredShifts.length}</strong><p>{uncoveredShifts.length > 0 ? 'Une intervention est nécessaire.' : 'Aucun quart publié sous-staffé.'}</p></div><div><span className="metric-label">BROUILLONS À PUBLIER</span><strong>{draftShifts.length}</strong><p>Préparés par les responsables.</p></div></div><div className="coverage-list">{coverage.length === 0 ? <div className="empty-state">Aucun quart à analyser sur la période.</div> : coverage.map((shift) => <article className={`coverage-row ${!shift.isCovered && shift.publicationStatus === 'Published' ? 'is-risk' : ''}`} key={shift.shiftId}><div className="coverage-date"><strong>{formatCoverageDate(shift.startUtc)}</strong><span>{formatTime(shift.startUtc)} – {formatTime(shift.endUtc)}</span></div><div className="coverage-site"><strong>{shift.siteName}</strong><span>{shift.publicationStatus === 'Draft' ? 'Brouillon' : 'Publié'}</span></div><div className="coverage-meter"><div className="coverage-meter-label"><span>Effectif</span><strong>{shift.assignedLifeguards}/{shift.requiredLifeguards}</strong></div><div className="coverage-track"><span style={{ width: `${Math.min(100, Math.round((shift.assignedLifeguards / shift.requiredLifeguards) * 100))}%` }} /></div></div><span className={`coverage-status ${shift.publicationStatus === 'Draft' ? 'draft' : shift.isCovered ? 'covered' : 'risk'}`}>{shift.publicationStatus === 'Draft' ? 'À publier' : shift.isCovered ? 'Couvert' : 'À compléter'}</span></article>)}</div></section>}
        {view === 'audit' && <section className="full-section audit-page"><div className="section-header"><div><h2>Journal des opérations</h2><p>Les actions sensibles de votre organisation, conservées pour la traçabilité.</p></div><div className="audit-header-actions"><span className="audit-count">{auditTotal} opération{auditTotal === 1 ? '' : 's'}</span><button className="secondary-button audit-export" onClick={() => { void exportAudit() }} disabled={auditExporting}>{auditExporting ? 'Export…' : 'Exporter CSV'}</button></div></div><form className="audit-filters" onSubmit={(event) => { event.preventDefault(); void searchAudit(1) }}><label>Recherche<input aria-label="Rechercher dans l’historique" value={auditFilters.q} onChange={(event) => setAuditFilters((filters) => ({ ...filters, q: event.target.value }))} placeholder="Action, objet, acteur ou détail" /></label><label>Action<select aria-label="Filtrer par action" value={auditFilters.action} onChange={(event) => setAuditFilters((filters) => ({ ...filters, action: event.target.value }))}><option value="">Toutes les actions</option>{Object.entries(auditActionLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Objet<select aria-label="Filtrer par objet" value={auditFilters.entityType} onChange={(event) => setAuditFilters((filters) => ({ ...filters, entityType: event.target.value }))}><option value="">Tous les objets</option>{['Organization', 'Site', 'Sector', 'Shift', 'Assignment', 'SwapRequest', 'Invitation', 'OrganizationMembership'].map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>Du<input aria-label="Date de début" type="date" value={auditFilters.from} onChange={(event) => setAuditFilters((filters) => ({ ...filters, from: event.target.value }))} /></label><label>Au<input aria-label="Date de fin" type="date" value={auditFilters.to} onChange={(event) => setAuditFilters((filters) => ({ ...filters, to: event.target.value }))} /></label><button className="primary-button audit-search" type="submit" disabled={auditLoading}>{auditLoading ? 'Recherche…' : 'Rechercher'}</button>{(auditFilters.q || auditFilters.action || auditFilters.entityType || auditFilters.from || auditFilters.to) && <button className="secondary-button audit-reset" type="button" disabled={auditLoading} onClick={() => { const cleared: AuditFilters = { q: '', action: '', entityType: '', from: '', to: '' }; setAuditFilters(cleared); void searchAudit(1, cleared) }}>Réinitialiser</button>}</form>{auditEntries.length === 0 ? <div className="empty-state">Aucune opération ne correspond à ces filtres.</div> : <div className="audit-list">{auditEntries.map((entry) => <div className="audit-row" key={entry.id}><div><strong>{formatAuditAction(entry.action)}</strong><span>{entry.entityType}{entry.details ? ` · ${entry.details}` : ''}</span></div><div><strong>{entry.actorName ?? 'Système'}</strong><span>{formatAuditDate(entry.createdAtUtc)}</span></div></div>)}</div>}{auditTotal > 20 && <div className="audit-pagination"><button className="secondary-button" disabled={auditPage <= 1 || auditLoading} onClick={() => { void searchAudit(auditPage - 1) }}>Précédent</button><span>Page {auditPage} sur {Math.ceil(auditTotal / 20)}</span><button className="secondary-button" disabled={auditPage >= Math.ceil(auditTotal / 20) || auditLoading} onClick={() => { void searchAudit(auditPage + 1) }}>Suivant</button></div>}</section>}
        {view === 'availability' && isManagement && <section className="full-section team-availability-page"><div className="availability-intro"><div><span className="drawer-kicker">DISPONIBILITÉS DE L’ÉQUIPE</span><h2>Planifier avec une équipe réellement disponible</h2><p>Consultez les déclarations de disponibilité de votre périmètre avant de publier ou de compléter un quart.</p></div><span className="availability-legend"><i className="available-dot" />Disponible</span></div><div className="team-availability-summary"><div><span className="metric-label">DÉCLARATIONS</span><strong>{teamAvailabilities.length}</strong><p>Sur les 14 prochains jours.</p></div><div><span className="metric-label">MEMBRES</span><strong>{new Set(teamAvailabilities.map((availability) => availability.employeeId)).size}</strong><p>Avec au moins une déclaration.</p></div><div><span className="metric-label">INDISPONIBILITÉS</span><strong className={teamAvailabilities.some((availability) => !availability.isAvailable) ? 'coverage-number warning' : 'coverage-number'}>{teamAvailabilities.filter((availability) => !availability.isAvailable).length}</strong><p>À prendre en compte pour la planification.</p></div></div><div className="team-availability-list">{teamAvailabilities.length === 0 ? <div className="empty-state">Aucune disponibilité déclarée sur la période.</div> : teamAvailabilities.map((availability) => <article className={`team-availability-row ${availability.isAvailable ? 'is-available' : 'is-unavailable'}`} key={availability.id}><span className="mini-avatar">{initials(availability.employeeName)}</span><div className="team-availability-person"><strong>{availability.employeeName}</strong><small>{availability.email}</small></div><div className="team-availability-date"><strong>{formatCertificationDate(availability.date)}</strong><span className={`status-tag ${availability.isAvailable ? 'approved' : 'pending'}`}>{availability.isAvailable ? 'Disponible' : 'Indisponible'}</span></div><p>{availability.note ?? (availability.isAvailable ? 'Aucune note ajoutée.' : 'Indisponibilité signalée par le membre.')}</p></article>)}</div></section>}
        {view === 'availability' && !isManagement && <section className="full-section availability-page"><div className="availability-intro"><div><span className="drawer-kicker">MES DISPONIBILITÉS</span><h2>Une vue claire pour mieux planifier</h2><p>Les changements sont enregistrés dans l’API et servent de signal au coordonnateur lors des prochains quarts.</p></div><span className="availability-legend"><i className="available-dot" />Disponible</span></div><div className="availability-grid">{weekDays.map((day) => { const saved = availabilities?.find((availability) => availability.date === day.date); const isAvailable = saved?.isAvailable ?? true; return <article className={`availability-card ${isAvailable ? 'is-available' : 'is-unavailable'}`} key={day.date}><div className="availability-date"><span>{day.day}</span><strong>{day.number}</strong></div><div><strong>{isAvailable ? 'Disponible' : 'Indisponible'}</strong><p>{saved?.note ?? (isAvailable ? 'Vous pouvez prendre un quart.' : 'Aucun quart à proposer ce jour.')}</p></div><button className="availability-toggle" disabled={availabilitySavingDate === day.date} onClick={() => { void toggleAvailability(day.date) }}>{availabilitySavingDate === day.date ? 'Enregistrement…' : isAvailable ? 'Déclarer indisponible' : 'Déclarer disponible'}</button></article> })}</div></section>}
      </div>
    </main>
    {sectorModalOpen && <div className="modal-backdrop" onClick={closeCreateSector}><form className="create-shift-modal sector-modal" onClick={(event) => event.stopPropagation()} onSubmit={submitSector} noValidate>
      <button className="drawer-close" type="button" onClick={closeCreateSector} aria-label="Fermer"><Icon name="close" /></button>
      <span className="drawer-kicker">PÉRIMÈTRES OPÉRATIONNELS</span><h2>Ajouter un secteur</h2>
      <p>Les secteurs servent à attribuer un périmètre précis aux chargés et à garder chaque décision dans la bonne équipe.</p>
      <div className="form-grid sector-form-grid"><label>Nom du secteur<input required autoFocus value={sectorDraft.name} onChange={(event) => setSectorDraft((draft) => ({ ...draft, name: event.target.value }))} placeholder="Secteur Ouest" /></label><label>Code court<input required maxLength={12} value={sectorDraft.code} onChange={(event) => setSectorDraft((draft) => ({ ...draft, code: event.target.value.toUpperCase() }))} placeholder="OUEST" /></label></div>
      {sectorError && <p className="form-error" role="alert">{sectorError}</p>}
      <div className="modal-actions"><button className="secondary-button" type="button" onClick={closeCreateSector}>Annuler</button><button className="primary-button" type="submit" disabled={sectorSubmitting}>{sectorSubmitting ? 'Ajout…' : 'Ajouter le secteur'}</button></div>
    </form></div>}
    {siteModalOpen && <div className="modal-backdrop" onClick={closeCreateSite}><form className="create-shift-modal site-modal" onClick={(event) => event.stopPropagation()} onSubmit={submitSite} noValidate>
      <button className="drawer-close" type="button" onClick={closeCreateSite} aria-label="Fermer"><Icon name="close" /></button>
       <span className="drawer-kicker">CATALOGUE DES INSTALLATIONS</span><h2>{siteEditing ? 'Modifier une piscine' : 'Ajouter une piscine'}</h2>
      <p>Décrivez l’installation pour que les quarts, les rattachements et les règles de saison restent cohérents.</p>
      <div className="form-grid site-form-grid">
        <label>Nom de la piscine<input required autoFocus value={siteDraft.name} onChange={(event) => setSiteDraft((draft) => ({ ...draft, name: event.target.value }))} placeholder="Piscine du Centre" /></label>
        <label>Type<select value={siteDraft.type} onChange={(event) => setSiteDraft((draft) => ({ ...draft, type: event.target.value as SiteDraft['type'] }))}><option value="Indoor">Intérieure</option><option value="Outdoor">Extérieure</option></select></label>
        <label>Adresse<input value={siteDraft.address} onChange={(event) => setSiteDraft((draft) => ({ ...draft, address: event.target.value }))} placeholder="123, rue Principale" /></label>
        <label>Quartier<input value={siteDraft.neighborhood} onChange={(event) => setSiteDraft((draft) => ({ ...draft, neighborhood: event.target.value }))} placeholder="Chomedey" /></label>
         <label>Fuseau horaire<select value={siteDraft.timeZoneId} onChange={(event) => setSiteDraft((draft) => ({ ...draft, timeZoneId: event.target.value }))}><option value="America/Toronto">Heure de l’Est — Montréal</option><option value="America/Vancouver">Heure du Pacifique — Vancouver</option><option value="America/Edmonton">Heure des Rocheuses — Edmonton</option><option value="America/Winnipeg">Heure du Centre — Winnipeg</option></select></label>
         <label>Saison d’ouverture<select value={siteDraft.season} onChange={(event) => setSiteDraft((draft) => ({ ...draft, season: event.target.value as SiteDraft['season'] }))}><option value="all-year">Ouverte toute l’année</option><option value="seasonal">Saison personnalisée</option></select></label>
         <label>Secteur<select value={siteDraft.sectorId ?? ''} onChange={(event) => setSiteDraft((draft) => ({ ...draft, sectorId: event.target.value }))}><option value="">Aucun secteur pour le moment</option>{availableSectors.filter((sector) => sector.isActive).map((sector) => <option key={sector.id} value={sector.id}>{sector.name} · {sector.code}</option>)}</select></label>
      </div>
      {siteDraft.season === 'seasonal' && <div className="season-fields"><span>Période d’ouverture</span><div className="form-grid"><label>Début<input required type="date" value={`2024-${siteDraft.startMonth.padStart(2, '0')}-${siteDraft.startDay.padStart(2, '0')}`} onChange={(event) => { const [, month, day] = event.target.value.split('-'); setSiteDraft((draft) => ({ ...draft, startMonth: month, startDay: day })) }} /></label><label>Fin<input required type="date" value={`2024-${siteDraft.endMonth.padStart(2, '0')}-${siteDraft.endDay.padStart(2, '0')}`} onChange={(event) => { const [, month, day] = event.target.value.split('-'); setSiteDraft((draft) => ({ ...draft, endMonth: month, endDay: day })) }} /></label></div></div>}
      <label className="site-checkbox"><input type="checkbox" checked={siteDraft.isMunicipal} onChange={(event) => setSiteDraft((draft) => ({ ...draft, isMunicipal: event.target.checked }))} /><span><strong>Installation municipale</strong><small>Identifie cette piscine comme une ressource de la Ville ou de votre municipalité.</small></span></label>
      {siteError && <p className="form-error" role="alert">{siteError}</p>}
      <div className="modal-actions"><button className="secondary-button" type="button" onClick={closeCreateSite}>Annuler</button><button className="primary-button" type="submit" disabled={siteSubmitting}>{siteSubmitting ? 'Enregistrement…' : siteEditing ? 'Enregistrer les changements' : 'Ajouter la piscine'}</button></div>
    </form></div>}
    ...
    {membershipEditing && <div className="modal-backdrop" onClick={closeMembershipEditor}><form className="auth-modal membership-modal" onClick={(event) => event.stopPropagation()} onSubmit={submitMembership} noValidate><button className="drawer-close" type="button" onClick={closeMembershipEditor} aria-label="Fermer"><Icon name="close" /></button><span className="drawer-kicker">ADMINISTRATION DE L’ÉQUIPE</span><h2>Modifier le rattachement</h2><p>Définissez le rôle et le périmètre opérationnel de {membershipEditing.employeeName}.</p><label className="auth-field">Rôle<select value={membershipDraft.role} onChange={(event) => setMembershipDraft((draft) => ({ ...draft, role: event.target.value as Role, siteId: '', sectorId: '' }))}>{[...new Set((currentUser.role === 'AquaticDirector' ? ['Lifeguard', 'PoolChief', 'SectorManager', 'AquaticDirector'] : currentUser.role === 'SectorManager' ? ['Lifeguard', 'PoolChief'] : ['Lifeguard']).concat(membershipEditing.role))].map((role) => <option key={role} value={role}>{roleLabel(role as Role)}</option>)}</select></label>{(membershipDraft.role === 'Lifeguard' || membershipDraft.role === 'PoolChief') && <label className="auth-field">Piscine de rattachement<select required value={membershipDraft.siteId} onChange={(event) => setMembershipDraft((draft) => ({ ...draft, siteId: event.target.value }))}><option value="">Choisir une piscine</option>{availableSites.map((site) => <option key={site.id} value={site.id}>{site.name}{site.neighborhood ? ` · ${site.neighborhood}` : ''}</option>)}</select></label>}{membershipDraft.role === 'SectorManager' && <label className="auth-field">Secteur de rattachement<select required value={membershipDraft.sectorId} onChange={(event) => setMembershipDraft((draft) => ({ ...draft, sectorId: event.target.value }))}><option value="">Choisir un secteur</option>{availableSectors.filter((sector) => sector.isActive).map((sector) => <option key={sector.id} value={sector.id}>{sector.name} · {sector.code}</option>)}</select></label>}{membershipDraft.role === 'AquaticDirector' && <p className="scope-hint">La régie aquatique voit toutes les piscines et tous les secteurs de l’organisation.</p>}{membershipError && <p className="form-error" role="alert">{membershipError}</p>}<div className="modal-actions"><button className="secondary-button" type="button" onClick={closeMembershipEditor}>Annuler</button><button className="primary-button" type="submit" disabled={membershipSubmitting}>{membershipSubmitting ? 'Mise à jour…' : 'Enregistrer'}</button></div></form></div>}
    ...
    {authModal && <div className="modal-backdrop" onClick={closeAuth}><form className="auth-modal" onClick={(event) => event.stopPropagation()} onSubmit={submitAuth} noValidate><button className="drawer-close" type="button" onClick={closeAuth} aria-label="Fermer"><Icon name="close" /></button><span className="drawer-kicker">{authModal === 'register' ? 'NOUVEL ESPACE' : authModal === 'accept-invitation' ? 'INVITATION D’ÉQUIPE' : authModal === 'password-reset-request' || authModal === 'password-reset-confirm' ? 'RÉCUPÉRATION DU COMPTE' : 'ACCÈS SÉCURISÉ'}</span><h2>{authModal === 'register' ? 'Créer votre espace Vigie' : authModal === 'accept-invitation' ? 'Rejoindre votre équipe' : authModal === 'password-reset-request' ? 'Réinitialiser votre mot de passe' : authModal === 'password-reset-confirm' ? 'Choisir un nouveau mot de passe' : 'Se connecter à Vigie'}</h2><p>{authModal === 'register' ? 'Votre centre aura son espace isolé, son coordonnateur et ses données privées.' : authModal === 'accept-invitation' ? 'Choisissez votre mot de passe pour activer votre accès au calendrier de votre organisation.' : authModal === 'password-reset-request' ? 'Saisissez votre courriel. Si un compte correspond, vous recevrez un lien sécurisé.' : authModal === 'password-reset-confirm' ? 'Le lien est valable 30 minutes et ne peut servir qu’une seule fois.' : 'Retrouvez le calendrier et les opérations de votre organisation.'}</p>{authModal === 'register' && <label className="auth-field">Nom de l’organisation<input required value={authForm.organizationName} onChange={(event) => setAuthForm((form) => ({ ...form, organizationName: event.target.value }))} placeholder="Centre aquatique Laval" /></label>}{(authModal === 'register' || authModal === 'accept-invitation') && <label className="auth-field">Votre nom<input required value={authForm.name} onChange={(event) => setAuthForm((form) => ({ ...form, name: event.target.value }))} placeholder="Marie Tremblay" /></label>}{authModal === 'register' && <><span className="auth-section-label">Votre centre</span><p className="auth-section-hint">Ces choix nous aident à préparer votre espace dès son ouverture.</p><div className="onboarding-grid"><label className="auth-field">Type de centre<select required value={authForm.organizationType} onChange={(event) => setAuthForm((form) => ({ ...form, organizationType: event.target.value }))}><option value="">Choisir un type</option>{organizationTypeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label className="auth-field">Nombre de piscines<select required value={authForm.poolCount} onChange={(event) => setAuthForm((form) => ({ ...form, poolCount: event.target.value }))}><option value="">Choisir un nombre</option>{poolCountOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label className="auth-field">Taille de l’équipe<select required value={authForm.teamSize} onChange={(event) => setAuthForm((form) => ({ ...form, teamSize: event.target.value }))}><option value="">Choisir une taille</option>{teamSizeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label className="auth-field">Votre priorité<select required value={authForm.primaryGoal} onChange={(event) => setAuthForm((form) => ({ ...form, primaryGoal: event.target.value }))}><option value="">Choisir une priorité</option>{primaryGoalOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label></div></>}{(authModal === 'login' || authModal === 'register' || authModal === 'password-reset-request') && <label className="auth-field">Courriel<input required type="email" value={authForm.email} onChange={(event) => setAuthForm((form) => ({ ...form, email: event.target.value }))} placeholder="vous@centre.ca" /></label>}{authModal !== 'password-reset-request' && <label className="auth-field">{authModal === 'password-reset-confirm' ? 'Nouveau mot de passe' : 'Mot de passe'}<input required type="password" minLength={12} value={authForm.password} onChange={(event) => setAuthForm((form) => ({ ...form, password: event.target.value }))} placeholder="12 caractères, une majuscule et un chiffre" /></label>}{authError && <p className="form-error" role="alert">{authError}</p>}<button className="primary-button full" type="submit" disabled={authSubmitting}>{authSubmitting ? 'Vérification…' : authModal === 'register' ? 'Créer mon espace' : authModal === 'accept-invitation' ? 'Activer mon compte' : authModal === 'password-reset-request' ? 'Envoyer le lien' : authModal === 'password-reset-confirm' ? 'Réinitialiser le mot de passe' : 'Se connecter'}</button>{authModal === 'login' && <button className="auth-switch" type="button" onClick={() => openAuth('password-reset-request')}>Mot de passe oublié ?</button>}{authModal === 'register' && <button className="auth-switch" type="button" onClick={() => openAuth('login')}>J’ai déjà un compte</button>}{authModal === 'password-reset-request' && <button className="auth-switch" type="button" onClick={() => openAuth('login')}>Retour à la connexion</button>}</form></div>}
    {passwordModalOpen && <div className="modal-backdrop" onClick={closePasswordModal}><form className="auth-modal" onClick={(event) => event.stopPropagation()} onSubmit={submitPassword} noValidate><button className="drawer-close" type="button" onClick={closePasswordModal} aria-label="Fermer"><Icon name="close" /></button><span className="drawer-kicker">SÉCURITÉ DU COMPTE</span><h2>Changer le mot de passe</h2><p>Votre mot de passe sera remplacé et toutes les autres sessions seront fermées.</p><label className="auth-field">Mot de passe actuel<input required type="password" value={passwordForm.current} onChange={(event) => setPasswordForm((form) => ({ ...form, current: event.target.value }))} /></label><label className="auth-field">Nouveau mot de passe<input required type="password" minLength={12} value={passwordForm.next} onChange={(event) => setPasswordForm((form) => ({ ...form, next: event.target.value }))} placeholder="12 caractères, une majuscule et un chiffre" /></label>{passwordError && <p className="form-error" role="alert">{passwordError}</p>}<div className="modal-actions"><button className="secondary-button" type="button" onClick={() => { void exportAccountData() }} disabled={accountExporting || accountDeleting}>{accountExporting ? 'Export…' : 'Télécharger mes données'}</button><button className="danger-button" type="button" onClick={() => { void deleteAccount() }} disabled={accountDeleting || accountExporting}>{accountDeleting ? 'Suppression…' : 'Supprimer mon compte'}</button><button className="secondary-button" type="button" onClick={closePasswordModal}>Annuler</button><button className="primary-button" type="submit" disabled={passwordSubmitting}>{passwordSubmitting ? 'Mise à jour…' : 'Enregistrer'}</button></div></form></div>}
    {certificationEditing && <div className="modal-backdrop" onClick={closeCertificationEditor}><form className="auth-modal" onClick={(event) => event.stopPropagation()} onSubmit={submitCertification} noValidate><button className="drawer-close" type="button" onClick={closeCertificationEditor} aria-label="Fermer"><Icon name="close" /></button><span className="drawer-kicker">SUIVI DES CERTIFICATIONS</span><h2>Mettre à jour une certification</h2><p>La date sera utilisée immédiatement lors de la prochaine assignation d’un quart.</p><div className="modal-shift"><strong>{certificationEditing.employeeName}</strong><span>{certificationEditing.type}</span></div><label className="auth-field">Date d’expiration<input required type="date" value={certificationDraft} onChange={(event) => setCertificationDraft(event.target.value)} /></label>{certificationError && <p className="form-error" role="alert">{certificationError}</p>}<div className="modal-actions"><button className="secondary-button" type="button" onClick={closeCertificationEditor}>Annuler</button><button className="primary-button" type="submit" disabled={certificationSubmitting}>{certificationSubmitting ? 'Enregistrement…' : 'Enregistrer la date'}</button></div></form></div>}
    {certificationCreateOpen && <div className="modal-backdrop" onClick={closeCreateCertification}><form className="auth-modal" onClick={(event) => event.stopPropagation()} onSubmit={submitCreateCertification} noValidate><button className="drawer-close" type="button" onClick={closeCreateCertification} aria-label="Fermer"><Icon name="close" /></button><span className="drawer-kicker">SUIVI DES CERTIFICATIONS</span><h2>Ajouter une certification</h2><p>Associez une certification à un membre afin que Vigie puisse contrôler son admissibilité.</p><label className="auth-field">Membre<select required value={certificationCreateDraft.employeeId} onChange={(event) => setCertificationCreateDraft((draft) => ({ ...draft, employeeId: event.target.value }))}>{teamMembers.filter((member) => member.role === 'Lifeguard').map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label><label className="auth-field">Type de certification<select required value={certificationCreateDraft.certificationTypeId} onChange={(event) => setCertificationCreateDraft((draft) => ({ ...draft, certificationTypeId: event.target.value }))}>{certificationTypes.map((type) => <option key={type.id} value={type.id}>{type.name}{type.isRequired ? ' · requise' : ''}</option>)}</select></label><label className="auth-field">Date d’expiration<input required type="date" value={certificationCreateDraft.expiresOn} onChange={(event) => setCertificationCreateDraft((draft) => ({ ...draft, expiresOn: event.target.value }))} /></label>{certificationCreateError && <p className="form-error" role="alert">{certificationCreateError}</p>}<div className="modal-actions"><button className="secondary-button" type="button" onClick={closeCreateCertification}>Annuler</button><button className="primary-button" type="submit" disabled={certificationCreateSubmitting}>{certificationCreateSubmitting ? 'Enregistrement…' : 'Ajouter la certification'}</button></div></form></div>}
