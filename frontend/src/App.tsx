...
function App() {
  ...
  const [siteDraft, setSiteDraft] = useState<SiteDraft>({ name: '', type: 'Indoor', timeZoneId: 'America/Toronto', season: 'all-year', startMonth: '6', startDay: '1', endMonth: '9', endDay: '1', address: '', neighborhood: '', isMunicipal: false })
  ...
  function openCreateSite() {
    ...
    setSiteError('')
    setSiteDraft({ name: '', type: 'Indoor', timeZoneId: 'America/Toronto', season: 'all-year', startMonth: '6', startDay: '1', endMonth: '9', endDay: '1', address: '', neighborhood: '', isMunicipal: false })
  ...
  async function submitSite(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const created = await vigieApi.createSite({ name, type: siteDraft.type, timeZoneId: siteDraft.timeZoneId, ...season, address: siteDraft.address?.trim() ?? '', neighborhood: siteDraft.neighborhood?.trim() ?? '', isMunicipal: siteDraft.isMunicipal })
      setSites((items) => [...items, created].sort((left, right) => left.name.localeCompare(right.name, 'fr')))
      setSiteModalOpen(false)
      flash(`${created.name} a été ajoutée à votre catalogue.`)
    } catch (error) {
  ...
  return <div className="app-shell">
    ...
    {siteModalOpen && <div className="modal-backdrop" onClick={closeCreateSite}><form className="create-shift-modal site-modal" onClick={(event) => event.stopPropagation()} onSubmit={submitSite} noValidate>
      ...
      <span className="drawer-kicker">CATALOGUE DES INSTALLATIONS</span><h2>Ajouter une piscine</h2>
      ...
      <div className="modal-actions"><button className="secondary-button" type="button" onClick={closeCreateSite}>Annuler</button><button className="primary-button" type="submit" disabled={siteSubmitting}>{siteSubmitting ? 'Ajout…' : 'Ajouter la piscine'}</button></div>
    </form></div>}
