...
import type { AuditEntryResponse, AuditPageResponse, AvailabilityResponse, CoverageResponse, CreateSiteInput, MembershipResponse, NotificationResponse, Role as ApiRole, SectorResponse, CertificationResponse, SiteResponse, ShiftResponse, SwapRequestResponse, TeamAvailabilityResponse, UserSummary } from './api/types'
  ...
  useEffect(() => {
    ...
    async function syncApi() {
      try {
        ...
        const [apiShifts, apiSwaps, employees, apiCertifications, apiSites, apiAvailabilities, apiSectors, apiMembers, apiAudit, apiNotifications, apiCoverage, apiTeamAvailabilities] = await Promise.all([vigieApi.shifts(calendarFrom, calendarTo), vigieApi.swaps(), vigieApi.employees(), vigieApi.certifications(), vigieApi.sites(), vigieApi.availability(), vigieApi.sectors(), vigieApi.members(), isManagement ? vigieApi.auditSearch() : Promise.resolve({ items: [], total: 0, page: 1, pageSize: 20 } satisfies AuditPageResponse), vigieApi.notifications(), isManagement ? vigieApi.coverage(calendarFrom, calendarTo) : Promise.resolve([]), isManagement ? vigieApi.teamAvailability() : Promise.resolve([])])
        if (!isCurrent()) return
