...
import type { AuditEntryResponse, AvailabilityResponse, CertificationResponse, SiteResponse, ShiftResponse, SwapRequestResponse, UserSummary } from './api/types'
...
type View = 'calendar' | 'swaps' | 'certifications' | 'team' | 'availability' | 'audit'
...
function App() {
  ...
  const [inviteForm, setInviteForm] = useState({ name: '', email: '', role: 'Lifeguard' })
  ...
  useEffect(() => {
    ...
    async function syncApi() {
      try {
        ...
        const [apiShifts, apiSwaps, employees, apiCertifications, apiSites, apiAvailabilities, apiAudit] = await Promise.all([vigieApi.shifts(), vigieApi.swaps(), vigieApi.employees(), vigieApi.certifications(), vigieApi.sites(), vigieApi.availability(), currentUserRole === 'Coordinator' ? vigieApi.audit() : Promise.resolve([])])
        if (!active) return
  ...
  function openInvite() { setInviteError(''); setInviteLink(''); setInviteForm({ name: '', email: '', role: 'Lifeguard' }); setInviteModalOpen(true) }
  async function submitInvite(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (inviteSubmitting) return
    setInviteSubmitting(true); setInviteError('')
    try {
      const invitation = await vigieApi.inviteMember(inviteForm.email, inviteForm.name, inviteForm.role)
      setInviteLink(invitation.inviteLink ?? invitation.inviteToken ?? '')
      setInviteForm({ name: '', email: '', role: 'Lifeguard' })
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileNavOpen ? 'is-open' : ''}`}>
      ...
      <nav aria-label="Navigation principale">
        ...
        {isCoordinator && <button className={view === 'audit' ? 'nav-item active' : 'nav-item'} onClick={() => { setView('audit'); setMobileNavOpen(false) }}><Icon name="history" /><span>Historique</span></button>}
      </nav>
