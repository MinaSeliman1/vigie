  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="content-wrap">
        <div className="page-heading"><div><p className="eyebrow">SEMAINE DU 7 AU 13 SEPTEMBRE 2026</p><h1>{view === 'calendar' ? (isManagement ? 'Vue équipe' : `Bonjour, ${currentUser.name.split(' ')[0]}`) : view === 'swaps' ? 'Demandes d’échange' : view === 'certifications' ? 'Certifications' : view === 'team' ? 'Équipe' : view === 'sites' ? 'Piscines municipales' : view === 'coverage' ? 'Couverture des quarts' : view === 'audit' ? 'Historique' : 'Disponibilités'}</h1><p className="page-subtitle">{view === 'calendar' ? (isManagement ? 'Gardez une vue claire sur les quarts et les remplacements.' : 'Voici vos quarts et les disponibilités de votre équipe.') : view === 'swaps' ? 'Chaque remplacement reste sous contrôle jusqu’à votre approbation.' : view === 'certifications' ? 'La bonne certification, au bon moment, pour chaque quart.' : view === 'team' ? 'Les personnes autorisées et leur état de préparation pour les quarts.' : view === 'sites' ? 'Les installations, secteurs et saisons d’ouverture de votre organisation.' : view === 'coverage' ? 'Identifiez les besoins à combler avant la prise de service.' : view === 'audit' ? 'Les opérations importantes de votre organisation, avec leur acteur et leur horodatage.' : 'Indiquez les jours où vous pouvez prendre un quart.'}</p></div>{view === 'calendar' && isManagement && <button className="primary-button" onClick={openCreateShift}><Icon name="plus" />Créer un quart</button>}{view === 'team' && isManagement && <button className="primary-button" onClick={openInvite}><Icon name="plus" />Inviter un membre</button>}</div>
