...
type Shift = { id: string; assignmentId?: string; siteId?: string; startUtc?: string; endUtc?: string; day: string; date: number; start: string; end: string; site: string; siteKind: string; status: 'assigné' | 'disponible'; lifecycleStatus?: string; publicationStatus?: string; colleagues: string[]; requiredLifeguards: number; assignments?: Array<{ id: string; employeeId: string; employeeName: string }> }
  ...
  useEffect(() => {
    ...
    async function syncApi() {
      try {
        ...
        const [apiShifts, apiSwaps, employees, apiCertifications, apiSites, apiAvailabilities, apiSectors, apiMembers, apiAudit, apiNotifications, apiCoverage, apiTeamAvailabilities] = await Promise.all([vigieApi.shifts(), vigieApi.swaps(), vigieApi.employees(), vigieApi.certifications(), vigieApi.sites(), vigieApi.availability(), vigieApi.sectors(), vigieApi.members(), isManagement ? vigieApi.auditSearch() : Promise.resolve({ items: [], total: 0, page: 1, pageSize: 20 } satisfies AuditPageResponse), vigieApi.notifications(), isManagement ? vigieApi.coverage() : Promise.resolve([]), isManagement ? vigieApi.teamAvailability() : Promise.resolve([])])
        if (!isCurrent()) return
