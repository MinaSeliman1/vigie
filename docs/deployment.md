...
Le service doit répondre à `https://<nom-du-service>.onrender.com/health` avec un JSON contenant `"status":"ok"`. Render fournit une URL publique et exécute maintenant `/health/ready` comme health check à chaque déploiement; cette route vérifie aussi que PostgreSQL est joignable. Pour un fournisseur d’observabilité, configurez `/health/ready` comme sonde de disponibilité et `/metrics` comme source Prometheus; ces routes ne renvoient aucune donnée personnelle. Les services Free se mettent en veille après 15 minutes sans trafic et peuvent prendre environ une minute à redémarrer; c’est attendu pour une offre à 0 $. Voir les [limites Render Free](https://render.com/docs/free).
