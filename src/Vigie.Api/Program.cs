...
var jwtKey = string.IsNullOrWhiteSpace(builder.Configuration["Jwt:Key"]) ? "vigie-development-key-change-in-production-2026" : builder.Configuration["Jwt:Key"]!;

builder.Services.AddSingleton<InMemoryVigieStore>();
builder.Services.AddSingleton<IEmployeeRepository>(sp => sp.GetRequiredService<InMemoryVigieStore>());
builder.Services.AddSingleton<ISiteRepository>(sp => sp.GetRequiredService<InMemoryVigieStore>());
builder.Services.AddSingleton<IShiftRepository>(sp => sp.GetRequiredService<InMemoryVigieStore>());
builder.Services.AddSingleton<ICertificationRepository>(sp => sp.GetRequiredService<InMemoryVigieStore>());
builder.Services.AddSingleton<ICertificationTypeRepository>(sp => sp.GetRequiredService<InMemoryVigieStore>());
builder.Services.AddSingleton<IAssignmentRepository>(sp => sp.GetRequiredService<InMemoryVigieStore>());
builder.Services.AddSingleton<ISwapRequestRepository>(sp => sp.GetRequiredService<InMemoryVigieStore>());
builder.Services.AddSingleton<IUnitOfWork>(sp => sp.GetRequiredService<InMemoryVigieStore>());
...
app.MapPost("/api/v1/auth/login", (LoginRequest request, InMemoryVigieStore store, JwtTokenService tokens) =>
{
...
app.MapGet("/api/v1/dashboard", (ClaimsPrincipal user, InMemoryVigieStore store) =>
{
...
app.MapGet("/api/v1/sites", (InMemoryVigieStore store) => Results.Ok(store.Sites.Select(ToSite).ToArray())).RequireAuthorization().WithTags("Sites");
app.MapPost("/api/v1/sites", (CreateSiteRequest request, InMemoryVigieStore store) =>
{
...
app.MapGet("/api/v1/shifts", (DateTimeOffset? from, DateTimeOffset? to, InMemoryVigieStore store) =>
{
...
app.MapPost("/api/v1/shifts", (CreateShiftRequest request, InMemoryVigieStore store) =>
{
    try
    {
...
app.MapGet("/api/v1/certifications", (ClaimsPrincipal user, InMemoryVigieStore store) =>
{
...
app.MapGet("/api/v1/employees", (InMemoryVigieStore store) => Results.Ok(store.Employees.Select(User).ToArray())).RequireAuthorization().WithTags("Employés");

app.MapPost("/api/v1/swap-requests", async (ClaimsPrincipal user, CreateSwapRequest request, RequestSwapService service, InMemoryVigieStore store, CancellationToken ct) =>
...
app.MapGet("/api/v1/swap-requests", (ClaimsPrincipal user, InMemoryVigieStore store) =>
{
...
app.MapPost("/api/v1/swap-requests/{requestId:guid}/approve", async (ClaimsPrincipal user, Guid requestId, ApproveSwapService service, InMemoryVigieStore store, CancellationToken ct) =>
...
app.MapPost("/api/v1/swap-requests/{requestId:guid}/reject", async (ClaimsPrincipal user, Guid requestId, RejectSwapService service, InMemoryVigieStore store, CancellationToken ct) =>
    (await service.ExecuteAsync(UserId(user), requestId, ct)).ToHttpResult(result => Results.Ok(ToSwap(result, store)))).RequireAuthorization(new AuthorizeAttribute { Roles = nameof(EmployeeRole.Coordinator) }).WithTags("Échanges");

app.MapGet("/api/v1/availability", (ClaimsPrincipal user, InMemoryVigieStore store) => Results.Ok(store.Availabilities.Where(a => a.EmployeeId == UserId(user)).OrderBy(a => a.Date).Select(a => new AvailabilityResponse(a.Id, a.EmployeeId, a.Date, a.IsAvailable, a.Note)).ToArray())).RequireAuthorization().WithTags("Disponibilités");
app.MapPut("/api/v1/availability", (ClaimsPrincipal user, AvailabilityRequest request, InMemoryVigieStore store) =>
{
...
static ShiftResponse ToShift(Shift shift, InMemoryVigieStore store) => new(shift.Id, shift.SiteId, store.Sites.Single(s => s.Id == shift.SiteId).Name, shift.StartUtc, shift.EndUtc, shift.RequiredLifeguards, store.Assignments.Where(a => a.ShiftId == shift.Id).Select(a => new AssignmentResponse(a.Id, a.ShiftId, a.EmployeeId, store.Employees.Single(e => e.Id == a.EmployeeId).Name)).ToArray());
static SwapRequestResponse ToSwap(SwapRequest request, InMemoryVigieStore store)
{
