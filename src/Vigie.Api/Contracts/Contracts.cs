...
public sealed record ShiftResponse(Guid Id, Guid SiteId, string SiteName, DateTimeOffset StartUtc, DateTimeOffset EndUtc, int RequiredLifeguards, IReadOnlyCollection<AssignmentResponse> Assignments);
...
public sealed record SwapRequestResponse(Guid Id, Guid AssignmentId, Guid ReceiverId, string ReceiverName, string ShiftLabel, string Status, DateTimeOffset RequestedAtUtc);
